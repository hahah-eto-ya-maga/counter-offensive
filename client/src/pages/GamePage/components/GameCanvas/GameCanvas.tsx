...
const GameCanvas: FC<GameCanvasProps> = ({ inputRef }) => {
  ...
  if (server.STORE.user) {
    ...
    switch (server.STORE.user.unit.personId) {
      ...
      case EGamerRole.middleTankMeh: {
        const { corpusR, rotateSpeed, speed } = entitiesConfig.middleTank;
        unit = new MiddleCorpus(x, y, angle, corpusR, speed, rotateSpeed);
        break;
      }
      ...
      case EGamerRole.heavyTankMeh: {
        const { corpusR, rotateSpeed, speed } = entitiesConfig.heavyTank;
        unit = new HeavyCorpus(x, y, angle, corpusR, speed, rotateSpeed);
        break;
      }
  ...
  const drawTanks = (tanks: ITank[]) => {
    tanks.forEach((tank) => {
      ...
      canvas?.spriteDir(img, tank.x - 3, tank.y + 3, ...corpus, tank.angle);
      canvas?.spriteDir(
        img,
        tank.x - 3,
        tank.y + 3,
  ...
  const drawBodies = (bodies: IBody[]) => {
    bodies.forEach((body) => {
      ...
      if (isMob) {
        ...
        canvas?.spriteDir(img, x - 1, y + 1, ...mobDead, Math.PI / 2 - angle);
        return;
      }
      switch (type) {
        case EBody.man: {
          canvas?.circle({ ...body, r: 0.5 }, "#F00");
          canvas?.spriteDir(img, x - 1, y + 1, ...manDead, Math.PI / 2 - angle);
          break;
        }
        case EBody.middleTank: {
          canvas?.spriteDir(img, x - 3, y + 3, ...corpusTank2Dead, -angle);
          break;
        }
        case EBody.middleTower: {
          canvas?.spriteDir(img, x - 3, y + 3, ...towerTank2Dead, -angle);
          break;
        }
        case EBody.heavyTank: {
          canvas?.spriteDir(img, x - 3, y + 3, ...corpusTank3Dead, -angle);
          break;
        }
        case EBody.heavyTower: {
          canvas?.spriteDir(img, x - 3, y + 3, ...towerTank3Dead, -angle);
          break;
        }
      }
    });
    ...
    if (game.serverUnit) {
      if (canvas) {
        ...
        if (unit instanceof General) {
          ...
          unit.rotate(
            Math.atan2(canvas.sy(mousePos.y) - y, canvas.sx(mousePos.x) - x)
          );
        }
        updateWIN();
      }
